...
if $INFRA_DRY_RUN || health_check "$app_id"; then record "$app_id" restart success; else record "$app_id" restart failed; die "$INFRA_EXIT_HEALTH" "restart health check failed"; fi
