...
if $INFRA_DRY_RUN || wait_for_health "$app_id"; then record "$app_id" rollback success; else record "$app_id" rollback failed; die "$INFRA_EXIT_HEALTH" "rollback health check failed"; fi
