            ...
            {list.map((r) => {
              ...
              return (
                <tr key={r.id}>
                  ...
                  <AdminTd>
                    {r.status}
                    {r.escalatedAt && <span className="ms-1 text-amber-600" title="Escalated">↑</span>}
                  </AdminTd>
                  <AdminTd>
                    {isSubmitted && (
                      <>
                        {isAdmin && (
                          <button type="button" onClick={() => handleAdminApprove(r.id)} disabled={!!loadingId} className="me-1 rounded border border-emerald-300 bg-emerald-50 px-2 py-1 text-xs text-emerald-800 disabled:opacity-50">Admin Approve</button>
                        )}
                        {!isAdmin && canManagerApprove && (
                          <button type="button" onClick={() => handleApprove(r.id)} disabled={!!loadingId} className="me-1 rounded border border-emerald-300 bg-emerald-50 px-2 py-1 text-xs text-emerald-800 disabled:opacity-50">Approve</button>
                        )}
                        {!isAdmin && (requiresAdmin || !eval_?.canManagerApprove) && (
                          <button type="button" onClick={() => handleEscalate(r.id)} disabled={!!loadingId} className="me-1 rounded border border-amber-300 bg-amber-50 px-2 py-1 text-xs text-amber-800 disabled:opacity-50">Escalate to Admin</button>
                        )}
