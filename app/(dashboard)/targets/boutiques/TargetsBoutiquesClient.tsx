...
export function TargetsBoutiquesClient() {
  const { t } = useT();
  const [scope, setScope] = useState<{ canEdit: boolean; boutiques: Boutique[] } | null>(null);
  ...
  useEffect(() => {
    fetch('/api/targets/scope', { cache: 'no-store' })
      ...
      .then((data) => {
        if (data.boutiques) {
          setScope({ canEdit: data.canEdit ?? false, boutiques: data.boutiques });
          if (data.boutiques.length > 0 && !boutiqueId) {
            setBoutiqueId(data.boutiques[0].id);
          }
        }
      })
      .catch(() => {});
  ...
  const load = useCallback(() => {
    ...
    fetch(`/api/targets/boutiques?${params}`, { cache: 'no-store' })
      .then((r) => r.json())
      .then(setRows)
      .catch(() => setRows([]))
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4">
      ...
      <PageHeader
        title={t('targetsManagement.boutiqueTargets')}
