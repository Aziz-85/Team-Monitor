      ...
      <OpsCard title={t('nav.admin.memberships')}>
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2">
          <input
            ...
            className="max-w-xs rounded-lg border border-border px-3 py-1.5 text-sm text-foreground"
          />
          <button type="button" onClick={onSearchSubmit} className="rounded-lg border border-border bg-surface px-3 py-1.5 text-sm text-foreground hover:bg-surface-subtle">
            {t('common.search')}
          </button>
          <button
            type="button"
...
function EditMembershipForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div>
        <label className="mb-1 block text-sm font-medium text-foreground">{t('common.role')}</label>
        <select value={role} onChange={(e) => setRole(e.target.value)} className="w-full rounded-lg border border-border px-3 py-2 text-sm text-foreground">
      ...
      <div className="flex items-center gap-2">
        <input type="checkbox" id="edit-canAccess" checked={canAccess} onChange={(e) => setCanAccess(e.target.checked)} className="h-4 w-4 rounded border-border" />
        <label htmlFor="edit-canAccess" className="text-sm text-foreground">{t('admin.memberships.canAccess')}</label>
      </div>
      {(role === 'MANAGER' || role === 'ADMIN' || role === 'SUPER_ADMIN') && (
        <div className="space-y-1 border-t border-border pt-2">
          <p className="text-xs font-medium text-muted">Manager permissions</p>
      ...
      <div className="flex justify-end gap-2 pt-2">
        <button type="button" onClick={onCancel} className="rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground">{t('common.cancel')}</button>
        <button type="submit" disabled={saving} className="rounded-lg bg-accent px-3 py-2 text-sm text-white hover:bg-accent/90 disabled:opacity-50">{saving ? '…' : t('common.save')}</button>
      </div>
    </form>
  );
}
