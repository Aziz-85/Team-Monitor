  ...
  return (
    <div className="p-4 md:p-6">
      <div className="mx-auto max-w-4xl">
        <h1 className="mb-6 text-xl font-semibold text-slate-900">{t('admin.administration.dashboardTitle')}</h1>

        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {ADMINISTRATION_CARDS.map((card) => (
            <Link key={card.href} href={card.href}>
              <OpsCard className="h-full transition-colors hover:bg-slate-50">
                <h3 className="mb-1 text-sm font-medium text-slate-900">{t(card.titleKey)}</h3>
                <p className="text-xs text-slate-600">{t(card.descKey)}</p>
              </OpsCard>
            </Link>
          ))}
        </div>
