...
function PeriodComparison({ current, previous, currentYear, comparisonYear }: { current: MonthlyPoint[]; previous: MonthlyPoint[]; currentYear: string; comparisonYear: string }) {
  const rows = current.map((row, index) => ({
    monthNo: row.month.slice(5),
    current: sar(row.netSalesHalalas),
    previous: sar(previous[index]?.netSalesHalalas ?? 0),
  }));
  const max = Math.max(...rows.flatMap((row) => [row.current, row.previous]), 1);
  const labels = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  return <div className={`mt-5 grid gap-2 ${rows.length<=3?'grid-cols-3':rows.length<=6?'grid-cols-3 md:grid-cols-6':'grid-cols-6 md:grid-cols-12'}`}>{rows.map((row) => <div key={row.monthNo} className="group text-center"><div className="flex h-32 items-end justify-center gap-1 rounded-xl bg-surface-subtle/60 px-1.5 pt-3"><div title={`${currentYear}: ${formatSarInt(row.current)}`} className="w-2.5 rounded-t-full bg-accent transition-all group-hover:opacity-80" style={{height:`${Math.max(row.current ? 5 : 0,row.current/max*100)}%`}}/><div title={`${comparisonYear}: ${formatSarInt(row.previous)}`} className="w-2.5 rounded-t-full bg-[#8aa0b8] transition-all group-hover:opacity-80" style={{height:`${Math.max(row.previous ? 5 : 0,row.previous/max*100)}%`}}/></div><p className="mt-2 text-[10px] font-bold text-muted">{labels[Number(row.monthNo)-1]}</p></div>)}</div>;
}
...
export function PerformanceIntelligenceClient() {
  ...
  const priorRange = period === 'custom' ? { from: previousYear(range.from), to: previousYear(range.to) } : { from: `${comparisonYear}${range.from.slice(4)}`, to: `${comparisonYear}${range.to.slice(4)}` };
  ...
  const comparisonCutoffYear = period === 'custom' ? previousYear(currentMonthKey).slice(0, 4) : comparisonYear;
  ...
  useEffect(() => {
    ...
    const priorSalesTo = includesCurrent ? `${period === 'custom' ? Number(currentMonth.slice(0,4))-1 : comparisonYear}${currentMonth.slice(4)}` : priorRange.to;
    Promise.all([
  ...
  const comparisonLabel = period === 'custom' ? priorRange.from.slice(0,4) : comparisonYear;
  const comparisonPeriodName = period === 'month' ? 'Monthly' : period === 'quarter' ? 'Quarterly' : period === 'half' ? 'Half-year' : period === 'year' ? 'Annual' : 'Custom period';
  ...
  return (
    <main className="relative mx-auto max-w-[1680px] space-y-5 overflow-hidden p-4 md:p-6">
      ...
      <header className="relative overflow-hidden rounded-[34px] border border-white/10 bg-[linear-gradient(118deg,#081b33_0%,#102e53_48%,#124d5b_115%)] p-5 text-white shadow-[0_30px_80px_-38px_rgba(3,18,38,.9)] md:p-7">
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select aria-label="Period type" value={period} onChange={(e) => updateQuery({ period: e.target.value, employee: null, compareYear: null })} className="h-11 rounded-2xl border border-white/15 bg-white/10 px-4 text-sm font-bold text-white backdrop-blur [color-scheme:dark]">
            ...
            {period !== 'custom' && <select aria-label="Comparison year" value={comparisonYear} onChange={(e)=>updateQuery({compareYear:e.target.value})} className="h-11 rounded-2xl border border-white/15 bg-white/10 px-4 text-sm font-bold text-white backdrop-blur [color-scheme:dark]">
              {Array.from(new Set([...yearHistory.map((row)=>row.year), String(Number(anchorYear)-1), String(Number(anchorYear)-2)])).filter((year)=>Number(year)<Number(anchorYear)).sort((a,b)=>Number(b)-Number(a)).map((year)=><option key={year} value={year}>{year===String(Number(anchorYear)-1)?`Previous year · ${year}`:`Compare with ${year}`}</option>)}
            </select>}
            {period === 'custom' ? <>
              ...
              <button aria-label="Previous period" className="grid h-11 w-11 place-items-center rounded-2xl border border-white/15 bg-white/10 text-lg transition hover:bg-white/15" onClick={() => updateQuery({ month: addMonths(month, period === 'year' ? -12 : period === 'half' ? -6 : period === 'quarter' ? -3 : -1), compareYear: null })}>←</button>
              <input aria-label="Anchor month" type="month" value={month} onChange={(e) => parseMonthKey(e.target.value) && updateQuery({ month: e.target.value, compareYear: null })} className="h-11 rounded-2xl border border-white/15 bg-white/10 px-3 text-sm font-bold text-white [color-scheme:dark]" />
              <button aria-label="Next period" className="grid h-11 w-11 place-items-center rounded-2xl border border-white/15 bg-white/10 text-lg transition hover:bg-white/15" onClick={() => updateQuery({ month: addMonths(month, period === 'year' ? 12 : period === 'half' ? 6 : period === 'quarter' ? 3 : 1), compareYear: null })}>→</button>
            </>}
          </div>
        </div>
      ...
      {activeTab === 'overview' && <>
        ...
        {period !== 'custom' && <article className="app-card rounded-[28px] p-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-accent">{comparisonPeriodName} year over year</p><h2 className="mt-1 text-xl font-black">{range.from} → {range.to} compared with {priorRange.from} → {priorRange.to}</h2><p className="mt-1 text-xs text-muted">Same-date cutoff is used when the selected period is still in progress.</p></div><div className="flex gap-5 text-xs"><span><i className="me-2 inline-block h-2.5 w-2.5 rounded-full bg-accent"/>{anchorYear}</span><span><i className="me-2 inline-block h-2.5 w-2.5 rounded-full bg-[#8aa0b8]"/>{comparisonLabel}</span></div></div><div className="mt-5 grid gap-3 md:grid-cols-3"><div className="rounded-2xl bg-surface-subtle p-4"><p className="text-xs text-muted">Current sales</p><p className="mt-2 text-2xl font-black">{formatSarInt(model.total)}</p></div><div className="rounded-2xl bg-surface-subtle p-4"><p className="text-xs text-muted">Comparison sales</p><p className="mt-2 text-2xl font-black">{formatSarInt(model.priorTotal)}</p></div><div className="rounded-2xl bg-surface-subtle p-4"><p className="text-xs text-muted">Sales movement</p><p className={`mt-2 text-2xl font-black ${(model.yoy??0)>=0?'text-emerald-700':'text-rose-700'}`}>{model.yoy==null?'—':`${model.yoy>=0?'+':''}${model.yoy.toFixed(1)}%`}</p></div></div><PeriodComparison current={current.monthly} previous={prior?.monthly??[]} currentYear={anchorYear} comparisonYear={comparisonLabel}/></article>}
