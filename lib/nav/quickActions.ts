...
/** Deep link to the daily sales ledger tab in the sales import hub. */
export const DAILY_SALES_LEDGER_HREF = '/admin/import/sales?section=ledger';
...
export const QUICK_ACTION_DEFS: QuickActionDefinition[] = [
  ...
  { key: 'dailySalesLedger', href: DAILY_SALES_LEDGER_HREF, titleKey: 'home.quickActions.dailySalesLedger.title', hintKey: 'home.quickActions.dailySalesLedger.hint' },
...
export function actionKeyFromPathname(pathname: string, section?: string | null): QuickActionKey | null {
  ...
  if (pathname === '/admin/import/sales' || pathname.startsWith('/admin/import/sales/')) {
    if (section === 'ledger') return 'dailySalesLedger';
    return null;
  }
