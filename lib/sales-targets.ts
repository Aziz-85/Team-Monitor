...
export function canEditSalesForDate(role: Role, dateStr: string): boolean {
  if (role === 'MANAGER' || role === 'AREA_MANAGER' || role === 'ADMIN' || role === 'SUPER_ADMIN') return true;
...
export async function canEditSalesForDateWithGrant(
  ...
  if (user.role === 'MANAGER' || user.role === 'AREA_MANAGER' || user.role === 'ADMIN' || user.role === 'SUPER_ADMIN') return true;
